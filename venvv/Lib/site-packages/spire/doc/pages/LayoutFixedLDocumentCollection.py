...
class LayoutFixedLDocumentCollection (LayoutCollection) :
    ...
    def IndexOf(self ,entity:'FixedLayoutDocument')->int:
        """
        Returns the zero-based index of the specified entity.
        """
        intPtrentity:c_void_p = entity.Ptr

        GetDllLibDoc().LayoutFixedLDocumentCollection_IndexOf.argtypes=[c_void_p ,c_void_p]
        GetDllLibDoc().LayoutFixedLDocumentCollection_IndexOf.restype=c_int
        ret = CallCFunction(GetDllLibDoc().LayoutFixedLDocumentCollection_IndexOf,self.Ptr, intPtrentity)
        return ret
