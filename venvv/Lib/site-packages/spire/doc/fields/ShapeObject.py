...
class ShapeObject (  Shape, ICompositeObject) :
    """
    ...
    def AdjustHandles(self)->'ShapeAdjustHandles':
        """
        Provides methods for adjusting the preset geometric shapes.
        """
        GetDllLibDoc().ShapeObject_get_AdjustHandles.argtypes=[c_void_p]
        GetDllLibDoc().ShapeObject_get_AdjustHandles.restype=c_void_p
        intPtr = CallCFunction(GetDllLibDoc().ShapeObject_get_AdjustHandles,self.Ptr)
        from spire.doc.fields.shapes.ShapeAdjustHandles import ShapeAdjustHandles
        ret = None if intPtr==None else ShapeAdjustHandles(intPtr)
        return ret
