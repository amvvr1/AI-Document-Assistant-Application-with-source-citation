...
class DocOleObject(ShapeObject, IDocumentObject):
    ...
    def SetNativeData(self ,nativeData:bytes):
        """
        Sets the native data.
        Args:
            nativeData:The native data.
        """
        list_address:c_void_p = cast((c_ubyte * len(nativeData)).from_buffer_copy(nativeData),c_void_p)
        length:c_int = len(nativeData)


        GetDllLibDoc().DocOleObject_SetNativeData.argtypes=[c_void_p ,c_void_p, c_int]
        CallCFunction(GetDllLibDoc().DocOleObject_SetNativeData,self.Ptr, list_address,length)
