...
class SdtDate (  SdtControlProperties) :
    ...
    def Type(self)->'SdtType':
        """
        Gets the type of the current instance, which is set to <see cref="SdtType.DatePicker"/>.
        """
        GetDllLibDoc().SdtDate_get_Type.argtypes=[c_void_p]
        GetDllLibDoc().SdtDate_get_Type.restype=c_int
        ret = CallCFunction(GetDllLibDoc().SdtDate_get_Type,self.Ptr)
