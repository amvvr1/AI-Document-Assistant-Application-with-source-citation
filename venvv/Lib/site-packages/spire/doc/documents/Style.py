...
class Style (  DocumentSerializable, IStyle) :
    ...
    def Istd(self)->int:
        ...
        """
        GetDllLibDoc().Style_get_Istd.argtypes=[c_void_p]
        GetDllLibDoc().Style_get_Istd.restype=c_int
        ret = CallCFunction(GetDllLibDoc().Style_get_Istd,self.Ptr)
        return ret
    ...
    def BuiltIn(self)->bool:
        """
        Gets a value indicating whether the style is built-in (not user-defined).
        
        returns:
            True if the style identifier is built-in styles; otherwise, false.
        """
        GetDllLibDoc().Style_get_BuiltIn.argtypes=[c_void_p]
        GetDllLibDoc().Style_get_BuiltIn.restype=c_bool
        ret = CallCFunction(GetDllLibDoc().Style_get_BuiltIn,self.Ptr)
        return ret

    def RemoveSelf(self):
        """
        Removes this style from the styles collection.
        """
        GetDllLibDoc().Style_RemoveSelf.argtypes=[c_void_p]
        CallCFunction(GetDllLibDoc().Style_RemoveSelf,self.Ptr)