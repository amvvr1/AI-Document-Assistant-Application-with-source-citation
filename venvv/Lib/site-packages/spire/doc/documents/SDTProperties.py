...
class SDTProperties (  DocumentSerializable) :
    ...
    def DataBinding(self)->'SdtDataBinding':
        """
        ...
        """
        GetDllLibDoc().SDTProperties_get_DataBinding.argtypes=[c_void_p]
        GetDllLibDoc().SDTProperties_get_DataBinding.restype=c_void_p
        intPtr = CallCFunction(GetDllLibDoc().SDTProperties_get_DataBinding,self.Ptr)
        ret = None if intPtr==None else SdtDataBinding(intPtr)
        return ret
