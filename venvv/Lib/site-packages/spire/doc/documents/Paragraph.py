...
class Paragraph (  BodyRegion, IParagraph) :
    """
    ...
    def FindAllString(self ,matchString:str,caseSensitive:bool,wholeWord:bool)->List['TextSelection']:
        ...
        GetDllLibDoc().Paragraph_FindAllString.argtypes=[c_void_p ,c_char_p,c_bool,c_bool]
        GetDllLibDoc().Paragraph_FindAllString.restype=IntPtrArray
        intPtrArray = CallCFunction(GetDllLibDoc().Paragraph_FindAllString,self.Ptr, matchStringPtr,caseSensitive,wholeWord)
    ...
    def FindAllPattern(self ,pattern:'Regex')->List[TextSelection]:
        ...
        GetDllLibDoc().Paragraph_FindAllPattern.argtypes=[c_void_p ,c_void_p]
        GetDllLibDoc().Paragraph_FindAllPattern.restype=IntPtrArray
        intPtrArray = CallCFunction(GetDllLibDoc().Paragraph_FindAllPattern,self.Ptr, intPtrpattern)
        ret = GetObjVectorFromArray(intPtrArray, TextSelection)
        return ret
...
#
    ...
    def AppendChart(self ,chartType:ChartType, width:float,height:float)->ShapeObject:
        """
    ...
    def AppendPicture(self ,imgStream:Stream)->DocPicture:
        ...
        GetDllLibDoc().Paragraph_AppendPictureI11.argtypes=[c_void_p ,c_void_p]
        GetDllLibDoc().Paragraph_AppendPictureI11.restype=c_void_p
        intPtr = CallCFunction(GetDllLibDoc().Paragraph_AppendPictureI11,self.Ptr, intPtrimgStream)
        ret = None if intPtr==None else DocPicture(intPtr)
        return ret
...
#
    ...
    def Frame(self)->'Frame':
        ...
        """
        GetDllLibDoc().Paragraph_get_Frame.argtypes=[c_void_p]
        GetDllLibDoc().Paragraph_get_Frame.restype=c_void_p
        intPtr = CallCFunction(GetDllLibDoc().Paragraph_get_Frame,self.Ptr)
        from spire.doc import Frame
        ret = None if intPtr==None else Frame(intPtr)
        return ret

    def GetText(self ,isShowFieldText:bool)->str:
        """
        Get the paragraph text.
        """
        
        GetDllLibDoc().Paragraph_GetText.argtypes=[c_void_p ,c_bool]
        GetDllLibDoc().Paragraph_GetText.restype=c_void_p
        ret = PtrToStr(CallCFunction(GetDllLibDoc().Paragraph_GetText,self.Ptr, isShowFieldText))
        return ret
