...
class StructureDocumentTagCell (  TableCell, IStructureDocument) :
    ...
    def RemoveSelfOnly(self):
        """
        Removes the node itself, while preserving its content within the document.
        """
        GetDllLibDoc().StructureDocumentTagCell_RemoveSelfOnly.argtypes=[c_void_p]
        CallCFunction(GetDllLibDoc().StructureDocumentTagCell_RemoveSelfOnly,self.Ptr)
