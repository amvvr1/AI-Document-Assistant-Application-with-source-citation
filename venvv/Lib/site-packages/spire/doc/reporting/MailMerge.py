...
from typing import TypeVar, Union, Generic, List, Tuple, Dict
...
#        GetDllLibDoc().MailMerge_ExecuteWidthNestedRegionDC(self.Ptr, intPtrdataSet,intPtrcommands)
    ...
    @dispatch
    def ExecuteWidthNestedRegion(self ,dataFile: str,commands: List[Dict]):
        """
            Executes the nested region.
        """
        dataFilePtr = StrToPtr(dataFile)
        list_Keys = []
        list_Values = []
        for tempDict in commands:
            if not isinstance(tempDict,dict):
                raise TypeError("The elements in the list must be of dictionary type")
            list_Keys.extend(tempDict.keys())
            list_Values.extend(tempDict.values())
