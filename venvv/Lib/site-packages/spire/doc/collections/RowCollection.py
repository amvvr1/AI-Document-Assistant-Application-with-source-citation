...
class RowCollection (  DocumentObjectCollection) :
    ...
    def get_Item(self ,index:int)->'TableRow':
        ...
        GetDllLibDoc().RowCollection_get_Item.restype= IntPtrWithTypeName
        intPtr = CallCFunction(GetDllLibDoc().RowCollection_get_Item,self.Ptr, index)
        ret = None if intPtr==None else self._create(intPtr)
        return ret

    def _create(self, intPtrWithTypeName: IntPtrWithTypeName) -> 'TableRow':
        """
        Creates a TableRow object from the given IntPtrWithTypeName.
        
        Args:
            intPtrWithTypeName: The IntPtrWithTypeName to create the TableRow from.
        ...
        if (strName == "Spire.Doc.Documents.StructureDocumentTagRow"):
