...
class Table (BodyRegion, ITable) :
    ...
    def TableFormat(self)->'RowFormat':
        ...
        """
        warnings.warn("This attribute has been deprecated, please call the 'Table.Format' attribute.", DeprecationWarning)
    ...
    def Format(self)->'TableFormat':
        ...
        Gets the table formatting after ResetCells call.
        """
        GetDllLibDoc().Table_get_Format.argtypes=[c_void_p]
        GetDllLibDoc().Table_get_Format.restype=c_void_p
        intPtr = CallCFunction(GetDllLibDoc().Table_get_Format,self.Ptr)
        ret = None if intPtr==None else TableFormat(intPtr)
        return ret
    ...
    @dispatch
    def ApplyStyle(self ,style:ITableStyle):
        """
        Applies a table style to the current table.

        Args:
            style:The table style to apply. Must not be null.
        """
        intPtrstyle:c_void_p = style.Ptr

        GetDllLibDoc().Table_ApplyStyleS.argtypes=[c_void_p ,c_void_p]
        CallCFunction(GetDllLibDoc().Table_ApplyStyleS,self.Ptr, intPtrstyle)
    ...
    def SetBorders(self ,borderStyle:'BorderStyle',lineWidth:float,color:'Color'):
        """
        ...
        """
        enumborderStyle:c_int = borderStyle.value
        intPtrcolor:c_void_p = color.Ptr

        GetDllLibDoc().Table_SetBorders.argtypes=[c_void_p ,c_int,c_float,c_void_p]
        CallCFunction(GetDllLibDoc().Table_SetBorders,self.Ptr, enumborderStyle,lineWidth,intPtrcolor)


    def SetBorder(self ,borderPos:'BorderPositions',borderStyle:'BorderStyle',lineWidth:float,color:'Color',isOverrideCellBorders:bool):
        """
		Sets the border for the specified border positions and style.
		
		Args:
			borderPos: The border positions to set.
			borderStyle: The border style to apply.
			lineWidth: The line width of the border.
			color: The color of the border.
			isOverrideCellBorders: Indicates whether to override cell borders.
        """
        enumborderPos:c_int = borderPos.value
        enumborderStyle:c_int = borderStyle.value
        intPtrcolor:c_void_p = color.Ptr

        GetDllLibDoc().Table_SetBorder.argtypes=[c_void_p ,c_int,c_int,c_float,c_void_p,c_bool]
        CallCFunction(GetDllLibDoc().Table_SetBorder,self.Ptr, enumborderPos,enumborderStyle,lineWidth,intPtrcolor,isOverrideCellBorders)

    def ClearBorders(self):
        """
		Clears the borders of all rows and cells in the table.
        """
        GetDllLibDoc().Table_ClearBorders.argtypes=[c_void_p]
        CallCFunction(GetDllLibDoc().Table_ClearBorders,self.Ptr)

    def SetShading(self ,texture:'TextureStyle',foregroundColor:'Color',backgroundColor:'Color'):
        """
        Applies shading settings (texture style, foreground, and background colors) to all rows and cells in the table.
        
        Args:
        texture: The texture style to apply (e.g., solid, gray, etc.).
        foregroundColor: The color for the foreground pattern.
        backgroundColor: The color for the background pattern.

        """
        enumtexture:c_int = texture.value
        intPtrforegroundColor:c_void_p = foregroundColor.Ptr
        intPtrbackgroundColor:c_void_p = backgroundColor.Ptr

        GetDllLibDoc().Table_SetShading.argtypes=[c_void_p ,c_int,c_void_p,c_void_p]
        CallCFunction(GetDllLibDoc().Table_SetShading,self.Ptr, enumtexture,intPtrforegroundColor,intPtrbackgroundColor)

    def ClearShading(self):
        """
        Clears the shading of all rows and cells in the table.
        """
        GetDllLibDoc().Table_ClearShading.argtypes=[c_void_p]
        CallCFunction(GetDllLibDoc().Table_ClearShading,self.Ptr)
