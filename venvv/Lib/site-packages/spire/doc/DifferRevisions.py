...
class DifferRevisions (SpireObject) :
    ...
    def MoveToRevisions(self)->List['DocumentObject']:
        """
        Gets the list of document objects to be moved to revisions.
        """
        GetDllLibDoc().DifferRevisions_get_MoveToRevisions.argtypes=[c_void_p]
        GetDllLibDoc().DifferRevisions_get_MoveToRevisions.restype=IntPtrArray
        intPtrArray = CallCFunction(GetDllLibDoc().DifferRevisions_get_MoveToRevisions,self.Ptr)
        ret = GetObjVectorFromArray(intPtrArray,DocumentObject)
        return ret
    ...
    def MoveFromRevisions(self)->List['DocumentObject']:
        """
        Gets the list of document objects that are being moved from revisions.
        """
        GetDllLibDoc().DifferRevisions_get_MoveFromRevisions.argtypes=[c_void_p]
        GetDllLibDoc().DifferRevisions_get_MoveFromRevisions.restype=IntPtrArray
        intPtrArray = CallCFunction(GetDllLibDoc().DifferRevisions_get_MoveFromRevisions,self.Ptr)
        ret = GetObjVectorFromArray(intPtrArray,DocumentObject)
        return ret
